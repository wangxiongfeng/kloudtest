...
//            getRtcManager().rtcEngine().disableLastmileTest();// 关闭网络测试
    ...
    public void muteAllExceptCoachMember(AgoraMember coachMember){

        getRtcManager().worker().getRtcEngine().muteAllRemoteAudioStreams(true);
        getRtcManager().worker().getRtcEngine().muteAllRemoteVideoStreams(true);
        getRtcManager().worker().getRtcEngine().muteRemoteVideoStream(coachMember.getUserId(),false);
        getRtcManager().worker().getRtcEngine().muteRemoteAudioStream(coachMember.getUserId(),false);
    }

    public void muteAllMember(){
        getRtcManager().worker().getRtcEngine().muteAllRemoteAudioStreams(true);
        getRtcManager().worker().getRtcEngine().muteAllRemoteVideoStreams(true);
    }
